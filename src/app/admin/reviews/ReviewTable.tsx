    ...
    const columns = useMemo<ColumnDef<ReviewData>[]>(() => [
            ...
            cell: ({row}) => {
                ...
                return (
                    <div className="flex gap-2 mr-6">
                        <img src={review.userId.avatarUrl || '/default-avatar.png'} alt="" className='h-6 w-6 object-cover rounded-full'/>
                        <div className="flex flex-col">
                          <h2 className='clamp-1'>{review.userId.fullName}</h2>
                          <h4 className="text-xs text-slate-500">@{review.userId.username}</h4>
                        </div>
                    </div>
                )
            },
        },
        {
          ...
          cell: ({ getValue }) => {
              ...
              return <div className='flex gap-1'>
                {[...Array(5)].map((_, i) => (
                  <i key={i} className={`ri-star-${i < rating ? 'fill' : 'line'} text-yellow-500`}></i>
                ))}
              </div>;
            }
        },
        {
            ...
            cell: ({ getValue }) => {
                ...
                return <span className='clamp-2'>{value || 'Không có bình luận'}</span>;
              }
          },
        {
          header: 'Ngày đăng',
          ...
          cell: ({ row }) => {
            ...
            return (
              <div className="flex gap-2 justify-center">
                <button
                  onClick={() => onEdit?.(review._id, review.rating, review.comment)}
