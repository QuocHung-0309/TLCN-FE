...
export default function TourForm({ tourId, mode }: TourFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "create") {
        await createTour.mutateAsync(formData as TourInput);
        showSuccess("Tạo tour thành công!");
        setTimeout(() => router.push("/admin/tours"), 1500);
      } else {
        await updateTour.mutateAsync(formData);
        showSuccess("Cập nhật tour thành công!");
        setTimeout(() => router.push("/admin/tours"), 1500);
      }
    } catch (error: any) {
      showError(error.response?.data?.message || error.message || "Không thể xử lý yêu cầu");
    }
  };
      ...
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-slate-600">
            {mode === "create" ? "Thêm một tour du lịch mới" : "Cập nhật thông tin tour"}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="mb-2 block font-semibold text-slate-900">Thời gian</label>
            <input
              type="text"
              name="time"
              value={formData.time}
        ...
        {/* Description */}
        <div>
          <label className="mb-2 block font-semibold text-slate-900">Mô tả</label>
          <textarea
            name="description"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="mb-2 block font-semibold text-slate-900">Giá người lớn (đ)</label>
            <input
              type="number"
              name="priceAdult"
          ...
          <div>
            <label className="mb-2 block font-semibold text-slate-900">Giá trẻ em (đ)</label>
            <input
              type="number"
              name="priceChild"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="mb-2 block font-semibold text-slate-900">Ngày khởi hành</label>
            <input
              type="date"
              name="startDate"
          ...
          <div>
            <label className="mb-2 block font-semibold text-slate-900">Ngày kết thúc</label>
            <input
              type="date"
              name="endDate"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="mb-2 block font-semibold text-slate-900">Số lượng</label>
            <input
              type="number"
              name="quantity"
          ...
          <div>
            <label className="mb-2 block font-semibold text-slate-900">Tối thiểu</label>
            <input
              type="number"
              name="min_guests"
          ...
          <div>
            <label className="mb-2 block font-semibold text-slate-900">Hiện tại</label>
            <input
              type="number"
              name="current_guests"
        ...
        {/* Images */}
        <div>
          <label className="mb-2 block font-semibold text-slate-900">Hình ảnh (URLs)</label>
